 ...
 * Enables live theme synchronization between CSS and Chart.js canvases.
 */
...
export function initYieldChart(canvasId) {
  ...
  // Create gradient fill dynamically using primary brand token
  ...
  return new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: [primaryColor, '#0284C7', '#F59E0B', '#10B981'],
        borderWidth: 3,
 ...
 */
export function initSparkline(canvasId, data, color = null) {
  const canvas = document.getElementById(canvasId);
  if (!canvas) return null;

  const strokeColor = color || getThemeColor('--primary-color', '#10B981');
